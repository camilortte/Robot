...
double calcularHeuristica(int robx,int roby){    
    ...
    return 10*(abs(robotx-salidax) + abs(roboty-saliday));
}
...
void generacionDeHijos(){
    ...
    cout<<"Se tendria que mover: "<<item<<endl<<"Costo heuristoco:"<<AEstretlla[item]->getF()<<endl;
    switch (item){
        case 0:
            moverArriba();
            break;
...
int puntajeMinimo(vector<Estado *> lista){
      ...
      double pminimo=estado->getF(); //suma del 1er estado
      int item=0;
      for (int i=0;i<arreglo.size();i++){ ///recorre
          cout<<"Se compara: "<<arreglo[i]->getF()<<"  CON "<<pminimo<<endl;
          if((arreglo[i]->getF())<pminimo){ //si encuentra //uno menor -> pminimo = ese menor
             pminimo=arreglo[i]->getF();
             item=i;
